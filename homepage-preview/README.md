...
Hero headlines use one color: white on navy, navy on light backgrounds. Do not split display headlines into red/blue or white/blue phrases. Editorial labels on light surfaces are slate/navy. Red is an optional, rare accent; it is not a heading or label color and does not need to appear in every section. Homepage story photos retain a bold 4px berry underline; impact-page photos remain unframed. Semantic validation errors retain their accessible error treatment. Photography stays natural in color. This direction supersedes earlier two-tone headline treatments.
